                                                                                                                             ...
                                                                                                                             delta : has-standard-input : arguments : standard-input :
                                                                                                                                ''
                                                                                                                                    COMMAND=${ if delta then scripts.verification.good else scripts.verification.bad } &&
                                                                                                                                        ...
                                                                                                                                        EXPECTED_STANDARD_OUTPUT=ALPHA_$( ${ scripts.util.identity } standard output ) &&
                                                                                                                                        EXPECTED_STANDARD_ERROR=$( ${ scripts.util.identity } standard error ) &&
                                                                                                                                        EXPECTED_SCRIPTS_FILE="test=${ environment-variable out }/scripts/test_util.hash=${ environment-variable out }/scripts/util/hash_util.identity=${ environment-variable out }/scripts/util/identity_util.write=${ environment-variable out }/scripts/util/write_verification.bad=${ environment-variable out }/scripts/verification/bad_verification.good=${ environment-variable out }/scripts/verification/good_verification.terminal=${ environment-variable out }/scripts/verification/terminal" &&
                                                                                                                                        EXPECTED_NO_SCRIPT_IDENTITY=EXPECTED_$( ARGUMENTS=$( ${ scripts.util.identity } ${ environment-variable "EXPECTED_IDENTITY" } no-script argument ) STANDARD_INPUT="" ${ scripts.util.identity } ) &&
                                                                                                                                        ...
                                                                                                                                        OBSERVED_SCRIPTS_FILE=BETA_$( ${ pkgs.coreutils }/bin/cat /build/$( ${ scripts.util.identity } scripts file ) ) &&
                                                                                                                                        OBSERVED_NO_SCRIPT_IDENTITY=$( ${ pkgs.coreutils }/bin/cat /build/$( ${ scripts.util.identity } ) ) &&
                                                                                                                                        OBSERVED_NO_SCRIPT_STANDARD_OUTPUT=OBSERVED_$( ${ pkgs.coreutils }/bin/cat /build/$( ${ scripts.util.identity } no-script standard output ) ) &&
                                                                                                                                        ...
                                                                                                                                        assert_equals "${ environment-variable "EXPECTED_NO_SCRIPT_IDENTITY" }" "${ environment-variable "OBSERVED_NO_SCRIPT_IDENTITY" }" "WTF"
                                                                                                        ...
                                                                                                        status : { pkgs , ... } : { environment-variable , has-standard-input , scripts , ... } :
                                                                                                            ''
                                                                                                                export ARGUMENTS=${ environment-variable "@" } &&
                                                                                                                    ...
                                                                                                                    ${ scripts.verification.terminal } $( ${ scripts.util.identity } no-script arguments ) | ${ scripts.util.write } /build/$( ${ scripts.util.identity } no-script standard output ) &&
