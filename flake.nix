                                                                                    ...
                                                                                    in
                                                                                        ''
                                                                                            export ${ cache-timestamp }=${ environment-variable "${ cache-timestamp }:=$( ${ pkgs.coreutils }/bin/date +%s )" } &&
                                                                                                ...
                                                                                                export ${ cache-epoch-hash }=$( ${ pkgs.coreutils }/bin/echo -n $(( ${ environment-variable cache-timestamp } / ${ builtins.trace "HI ${ builtins.typeOf populate } ${ builtins.concatStringsSep "," ( builtins.attrNames populate ) } " "60" } )) $( ${ pkgs.coreutils }/bin/whoami ) | ${ pkgs.coreutils }/bin/sha512sum | ${ pkgs.coreutils }/bin/cut --bytes -0 ) &&
