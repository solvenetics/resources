                                                                                                                                        ...
                                                                                                                                        strip
                                                                                                                                            ''
                                                                                                                                                export COMMAND=${ command } &&
                                                                                                                                                    ...
                                                                                                                                                    export STANDARD_INPUT=${ if has-standard-input then standard-input else "" } &&
                                                                                                                                                    EXPECTED_STATUS=${ builtins.toString ( if delta then 0 else 64 ) } &&
                                                                                                                                                    EXPECTED_ALPHA_STANDARD_OUTPUT=${ if delta then "\/build\/[a-zA-Z0-9]{8}\.resource\/target" else "\/build\/[a-zA-Z0-9]{8}\.broken\/target" } &&
                                                                                                                                                    EXPECTED_ALPHA_STANDARD_ERROR="${ if delta then "" else "We were unable to complete initiation:  \/build/[a-zA-Z0-9]{8}\.broken" }" &&
                                                                                                                                                    EXPECTED_ALPHA_LOG="_init" &&
                                                                                                                                                    assert_status_code ${ environment-variable "EXPECTED_STATUS" } "${ if has-standard-input then "${ pkgs.coreutils }/bin/echo ${ environment-variable "STANDARD_INPUT" } alpha | " else "" }${ environment-variable "COMMAND" } ${ environment-variable "ARGUMENTS" } alpha > /build/$( ${ scripts.util.identity } alpha standard output file ) 2> /build/$( ${ scripts.util.identity } alpha standard error file )" &&
                                                                                                                                                    OBSERVED_ALPHA_STANDARD_OUTPUT=$( ${ pkgs.coreutils }/bin/cat /build/$( ${ scripts.util.identity } alpha standard output file ) ) &&
                                                                                                                                                    OBSERVED_ALPHA_STANDARD_ERROR=$( ${ pkgs.coreutils }/bin/cat /build/$( ${ scripts.util.identity } alpha standard error file ) ) &&
                                                                                                                                                    OBSERVED_ALPHA_LOG=$( ${ pkgs.coreutils }/bin/cat $( ${ pkgs.coreutils }/bin/cat /build/$( ${ scripts.util.identity } alpha standard output file ) ) )
                                                                                                                                                    assert_matches ${ environment-variable "EXPECTED_ALPHA_STANDARD_OUTPUT" } ${ environment-variable "OBSERVED_ALPHA_STANDARD_OUTPUT" } "YES" &&
                                                                                                                                                    ${ if delta then "assert_equals" else "assert_matches" } "${ environment-variable "EXPECTED_ALPHA_STANDARD_ERROR" }" "${ environment-variable "OBSERVED_ALPHA_STANDARD_ERROR" }" "YES" &&
                                                                                                                                                    assert_equals ${ environment-variable "EXPECTED_ALPHA_LOG" } ${ environment-variable "OBSERVED_ALPHA_LOG" }
                                                                                                                                            '' ;
                                                                                                                        in
                                                                                                                            [
                                                                                                                                ( script true true )
